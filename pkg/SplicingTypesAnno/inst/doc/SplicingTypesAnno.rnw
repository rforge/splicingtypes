...
\item \textbf{geneName}: the gene name from gtf/gff file. Specifically, for gtf file,
      ...
      e.g., gene_id Pnpla7_chr2_+_intron_30: for intron retention. \\
...
mm9.pnpla7.gtfFile <- system.file("extdata", "mm9_Pnpla7.gtf", 
...
selectGene <- "Pnpla7"
sample1.c <- splicingCount(selectGene, bam.1, result.GRange,
                      sampleName="liver_ctr",
                      sampleID=1
                      )
sample2.c <- splicingCount(selectGene, bam.2, result.GRange,
                      sampleName="liver_ko",
                      sampleID=2
                      )
@
